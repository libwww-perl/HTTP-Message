...
for (1) {
    ...
    note sprintf "Encoded size %d bytes after round %d", length $compressed,
        $_;
}
...
is $response->max_body_size, 1024 * 1024,
    "The default maximum body size holds";

$response->max_body_size( 512 * 1024 );
is $response->max_body_size, 512 * 1024,
    "We can change the maximum body size";
...
like(
    $err, qr/more than 524288 octets/,
...
# Setting a limit must not break the brotli responses that fit under it.
{
    ...
    is(
        $small_response->decoded_content( raise_error => 1 ), 'hello world',
