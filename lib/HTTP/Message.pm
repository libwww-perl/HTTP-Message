...
eval "require $HTTP::URI_CLASS";
die $@ if $@;

*_utf8_downgrade = defined(&utf8::downgrade)
    ? sub {
    utf8::downgrade( $_[0], 1 )
        or Carp::croak("HTTP::Message content must be bytes");
    }
    : sub {
    };
...
sub _set_content {
    ...
    if ( !ref( $_[1] ) && ref( $self->{_content} ) eq "SCALAR" ) {
        ...
        die "Can't set content to be a scalar reference"
...
sub content_charset {
    ...
    if ( $self->content_is_xml ) {
        ...
        my $encoding = IO::HTML::find_charset_in(
            $$cref,
            {
                encoding    => 1,
                need_pragma => 0
...
sub decoded_content {
    ...
    eval {
        ...
        if ( defined $content_limit ) {
            %limiter_options
                = ( LimitOutput => 1, Bufsize => $content_limit );
        }
            ...
            for my $ce ( reverse split( /\s*,\s*/, lc($h) ) ) {
                ...
                if ( $ce eq "gzip" || $ce eq "x-gzip" ) {
                    ...
                    my ($i) = Compress::Raw::Zlib::Inflate->new(
                        %limiter_options,
                        ConsumeInput => 0
                        , # overridden by Zlib if we have %limiter_options :-(
                        WindowBits =>
                    ...
                    $res == Compress::Raw::Zlib::Z_BUF_ERROR()
                        and Carp::croak(
                        "Decoded content would be larger than $content_limit octets"
                        );
                    ...
                    if ( defined $content_limit ) {
                        ...
                        $@
                            ...
                            and Carp::croak(
                            "Can't unbrotli content: it is corrupt, or would decode to more than $content_limit octets"
                            );
                    }
                    else {
                ...
                {    # not really C-T-E, but should be harmless
                    require MIME::Base64;
...
sub decode {
    ...
    if (
        defined( my $content = $self->decoded_content( charset => "none" ) ) )
    {
        $self->remove_header(
...
sub encode {
    ...
    Carp::croak("Can't encode multipart/* messages")
        if $self->content_type =~ m,^multipart/,;
    Carp::croak("Can't encode message/* messages")
    ...
    for my $encoding (@enc) {
        if ( $encoding eq "identity" || $encoding eq "none" ) {
            ...
            IO::Compress::Deflate::deflate( \$content, \$output )
                or die
                "Can't deflate content: $IO::Compress::Deflate::DeflateError";
            $content = $output;
        }
            ...
            IO::Compress::Bzip2::bzip2( \$content, \$output )
                or die
                "Can't bzip2 content: $IO::Compress::Bzip2::Bzip2Error";
            $content = $output;
        }
...
sub dump {
    ...
    if ( !ref($content) ) {
        ...
        $content
            =~ s/([\0-\11\13-\037\177-\377])/sprintf('\\x%02X',ord($1))/eg;
...
sub parts {
    ...
    if (
        defined(wantarray)
        && ( !exists $self->{_parts} || ref( $self->{_content} ) eq "SCALAR" )
    ) {
        $self->_parts;
    }
    my $old = $self->{_parts};
...
sub _parts {
    ...
    if ( $ct =~ m,^multipart/, ) {
        ...
        my @h = HTTP::Headers::Util::split_header_words(
